...
class ResolutorLaberinto:
    def __init__(self, size: int, cadena_laberinto: str, retraso_ms: int):
        self.size = size
        self.laberinto = [list(cadena_laberinto[i:i+size]) for i in range(0, len(cadena_laberinto), size)]
    ...
    def _mover_a(self, x: int, y: int, visitados: set):
        old_x, old_y = self.posicion_actual
        
        if self.laberinto[old_x][old_y] not in ['0', 'X']:
            if (old_x, old_y) in visitados:
                self.laberinto[old_x][old_y] = 'o'
            else:
                self.laberinto[old_x][old_y] = ' '
...
def main():
    size = int(input("Ingrese el size del laberinto (n x n): "))
    ...
    laberinto = ""
    for i in range(size):
        fila = input(f"Ingrese fila {i + 1}: ")
        if len(fila) != size:
            print("Error: La fila ingresada no coincide con el size del laberinto.")
            return
        laberinto += fila
    
    retraso = int(input("\nIngrese el retraso en ms entre cada movimiento: "))
    resolutor = ResolutorLaberinto(size, laberinto, retraso)
    resolutor.resolver()
